...
// GPT Images supplies distinct backdrops. No face retouching or regeneration.
// Trade-show previews use scripts/build_trade_show_social.py.
// The main preview is a separate GPT Images edit; this script only renders members.
...
(async () => {
  ...
  try {
    ...
    async function render(html, output) {
      ...
      const clipped = await page.locator('h1, p, .footer').evaluateAll(elements => elements.some(el => {
        const box = el.getBoundingClientRect();
        return box.left < 0 || box.right > 1200 || box.top < 0 || box.bottom > 630 || el.scrollWidth > el.clientWidth;
      }));
      if (clipped) throw new Error(`Text overflows in ${output}`);
    ...
    for (const member of team.members) {
      ...
      *{box-sizing:border-box}body{margin:0;width:1200px;height:630px;background:#090909;color:#eee1c2;font-family:Lato,sans-serif;display:flex;align-items:center;gap:66px;padding:70px 90px}
      .background{position:absolute;inset:0;width:1200px;height:630px;object-fit:cover;z-index:0}.portrait,.copy{position:relative;z-index:1}
      ...
      </style></head><body><img class="background" src="${data('static/'+(member.social_backdrop || 'assets/team-social/backdrop.png'),'image/png')}" alt=""><div class="portrait"><div class="crop"><img src="${data('static/team/'+member.photo,'image/jpeg')}" alt="${name}"></div></div><div class="copy"><div class="brand"><img src="${data('static/assets/ce_logo_full.png','image/png')}" alt="${escape(team.company)}"></div><h1>${name}</h1><p>${escape(member.title)}</p><div class="rule"></div><div class="footer">Your wholesale jewelry contact<br><span style="display:block;margin-top:12px;color:#958972;font-size:15px">LOS ANGELES · SINCE 1992</span></div></div></body></html>`;
      const output = path.join(root,'static',member.social_image);
      await render(html, output);
    }
