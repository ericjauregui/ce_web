...
class FrontendContractTests(BaseWebTest):
    ...
    def test_homepage_catalog_density_dock_and_grid_contracts(self) -> None:
        css = self.load_site_css()
        body = self.client.get("/").get_data(as_text=True)
        condensed = re.sub(r"\s+", " ", css)

        self.assertIn("class=\"catalog-density-rail\"", body)
        self.assertIn("data-density-rail", body)
        self.assertIn("aria-label=\"Catalog zoom controls\"", body)
        self.assertIn("aria-hidden=\"true\"", body)
        self.assertIn("data-density-track", body)
        self.assertIn("data-density-thumb", body)
        self.assertIn('data-density-action="zoom-in"', body)
        self.assertIn('data-density-action="zoom-out"', body)
        self.assertIn('aria-label="Zoom in catalog grid"', body)
        self.assertIn('aria-label="Zoom out catalog grid"', body)
        self.assertIn('role="slider"', body)
        self.assertIn('aria-orientation="vertical"', body)
        self.assertIn('aria-label="Catalog zoom level"', body)
        self.assertIn('aria-valuemin="1"', body)
        self.assertIn('class="product-grid-row" data-density-grid', body)
        self.assertNotIn("row-cols-2 row-cols-md-3 row-cols-xl-4", body)
        self.assertIn(".catalog-density-rail {", css)
        self.assertIn(".catalog-density-rail.is-visible {", css)
        self.assertIn(".catalog-density-track {", css)
        self.assertIn(".catalog-density-thumb {", css)
        self.assertIn(".product-grid-row {", css)
        self.assertIn("top: 50%;", css)
        self.assertIn("transform: translate3d(-.35rem, -50%, 0);", css)
        self.assertIn("transform: translate3d(0, -50%, 0);", css)
        self.assertIn("background: transparent;", css)
        self.assertIn("background: rgb(var(--gold-rgb) / 0.9);", css)
        self.assertIn("width: .16rem;", css)
        self.assertIn("height: clamp(7.25rem, 20vh, 11rem);", css)
        self.assertNotIn("linear-gradient(180deg, rgb(var(--gold-rgb) / 0.7)", css)
        self.assertIn(
            "grid-template-columns: repeat(var(--product-grid-columns, 2), minmax(0, 1fr));",
            css,
        )
        self.assertNotIn('.product-grid-row[data-density="2"] {', css)
        self.assertNotIn('.product-grid-row[data-density="10"] {', css)
        self.assertIn('.product-grid-row[data-density-tier="1"] {', css)
        self.assertIn('.product-grid-row[data-density-tier="3"] {', css)
        self.assertIn(".catalog-density-btn:disabled { opacity: .42;", condensed)
        self.assertIn("opacity: 0;", css)
        self.assertIn("visibility: hidden;", css)
        self.assertIn("pointer-events: none;", css)

    def test_catalog_density_script_uses_dynamic_active_range_and_resyncs_open_drawers(self) -> None:
        script = (Path(webapp.BASE_DIR) / "static" / "js" /
                  "catalog.js").read_text(encoding="utf-8")

        self.assertIn('const PRODUCT_GRID_SELECTOR = "[data-density-grid]";', script)
        self.assertIn("const CATALOG_DENSITY_MIN = 1;", script)
        self.assertIn("const CATALOG_DENSITY_MAX = 20;", script)
        self.assertIn("const DEFAULT_CATALOG_DENSITY = 6;", script)
        self.assertIn("const CATALOG_DENSITY_RESPONSIVE_BREAKPOINTS = [", script)
        self.assertIn("defaultColumns:", script)
        self.assertIn("maxColumns: 19", script)
        self.assertIn("maxColumns: 16", script)
        self.assertIn("maxColumns: 13", script)
        self.assertIn("maxColumns: 11", script)
        self.assertIn("maxColumns: 8", script)
        self.assertIn("maxColumns: 6", script)
        self.assertIn("maxColumns: 4", script)
        self.assertIn("defaultColumns: 6", script)
        self.assertIn("defaultColumns: 5", script)
        self.assertIn("defaultColumns: 4", script)
        self.assertIn("defaultColumns: 3", script)
        self.assertIn("defaultColumns: 2", script)
        self.assertNotIn("defaultColumns: 8", script)
        self.assertNotIn("defaultColumns: 7", script)
        self.assertIn("minColumns: 2", script)
        self.assertIn("minGapRem: 0.34", script)
        self.assertIn("minGapRem: 0.38", script)
        self.assertIn("minGapRem: 0.42", script)
        self.assertIn("minGapRem: 0.46", script)
        self.assertIn("minGapRem: 0.5", script)
        self.assertIn("minGapRem: 0.58", script)
        self.assertIn("minGapRem: 0.68", script)
        self.assertIn("minCardWidth: 140", script)
        self.assertIn("minCardWidth: 148", script)
        self.assertIn("minCardWidth: 160", script)
        self.assertIn("minCardWidth: 168", script)
        self.assertIn("minCardWidth: 188", script)
        self.assertIn("minCardWidth: 172", script)
        self.assertIn("minCardWidth: 132", script)
        self.assertNotIn("minColumns: 1", script)
        self.assertNotIn("defaultColumns: 1", script)
        self.assertNotIn("window.localStorage", script)
        self.assertIn("function getCatalogDensityEffectiveMaxColumns(profile, availableWidth) {", script)
        self.assertIn("const totalGapWidth = Math.max(0, columnCount - 1) * minGapPixels;", script)
        self.assertIn("if (cardWidth >= profile.minCardWidth) {", script)
        self.assertIn("function getCatalogDensityActiveMaxValue(profile, effectiveMaxColumns) {", script)
        self.assertIn("function getDefaultCatalogDensityValueForRange({ profile, effectiveMaxColumns, activeMaxValue }) {", script)
        self.assertIn("function getCatalogDensityRangeState(grid = getCatalogDensityGrids()[0]) {", script)
        self.assertIn("const targetColumns = clampNumber(profile.defaultColumns, profile.minColumns, effectiveMaxColumns);", script)
        self.assertIn("for (let value = CATALOG_DENSITY_MIN; value <= activeMaxValue; value += 1) {", script)
        self.assertIn("function syncCatalogDensityGridLayout() {", script)
        self.assertIn('grid.style.setProperty("--product-grid-columns", String(metrics.columnCount));', script)
        self.assertIn('grid.style.setProperty("--product-grid-gap", metrics.gapRem);', script)
        self.assertIn('grid.dataset.densityTier = String(metrics.compactTier);', script)
        self.assertIn("document.querySelectorAll(\".product-card.is-open\").forEach(syncDrawerHeight);", script)
        self.assertIn("thumb.style.setProperty(\"--catalog-density-progress\"", script)
        self.assertIn('thumb.setAttribute("aria-valuemax", String(catalogDensityActiveMaxValue));', script)
        self.assertIn("zoomInButton.disabled = catalogDensityValue <= CATALOG_DENSITY_MIN;", script)
        self.assertIn("zoomOutButton.disabled = catalogDensityValue >= catalogDensityActiveMaxValue;", script)
        self.assertIn("track?.addEventListener(\"pointerdown\"", script)
        self.assertIn("thumb?.addEventListener(\"keydown\"", script)
        self.assertIn("getCatalogDensityFromPointer(event.clientY, track)", script)
        self.assertIn("if (!(event.ctrlKey || event.metaKey)) return;", script)
        self.assertIn("event.preventDefault();", script)
        self.assertIn("densityWheelCooldownId = window.setTimeout(() => {", script)
        self.assertIn("const densityProgress = getCatalogDensityNormalizedProgress(value);", script)
        self.assertIn('return getCatalogDensityNormalizedProgress(value, maxValue) * 100;', script)
        self.assertIn('const nextValue = CATALOG_DENSITY_MIN + Math.round(ratio * (catalogDensityActiveMaxValue - CATALOG_DENSITY_MIN));', script)
        self.assertIn('applyCatalogDensity(catalogDensityActiveMaxValue);', script)
        self.assertIn('const stickyHeaders = Array.from(document.querySelectorAll(".sticky-section-header"));', script)
        self.assertIn('return stickyHeaders[1] || stickyHeaders[0] || document.querySelector(".section-header-row");', script)
        self.assertIn('const target = getCatalogDensityVisibilityTarget();', script)
        self.assertIn('elements.rail.classList.toggle("is-visible", isVisible);', script)
        self.assertIn('window.addEventListener("scroll", queueCatalogDensityVisibilitySync, { passive: true });', script)
        self.assertIn('window.addEventListener("resize", queueCatalogDensityVisibilitySync, { passive: true });', script)
        self.assertIn('nudgeCatalogDensity(event.deltaY < 0 ? -1 : 1);', script)
        self.assertIn('rail.setAttribute("title", "Catalog zoom controls. Plus zooms in to fewer columns, minus zooms out to more columns. Drag the thumb or hold Ctrl or Cmd and use the mouse wheel to adjust.");', script)
        self.assertIn("let catalogDensityActiveMaxValue = CATALOG_DENSITY_MAX;", script)
        self.assertIn("syncCatalogDensityState({ resetToDefault: true });", script)
        self.assertIn("catalogDensityValue = resetToDefault", script)
        self.assertIn("syncCatalogDensityState();", script)
        self.assertIn("initializeCatalogDensity();", script)
        self.assertIn("document.addEventListener(\"ce:content-replaced\", () => {", script)
        self.assertIn("syncCatalogDensityGridLayout();", script)
    ...
    def test_inline_reels_pause_out_of_view_and_autoplay_next_preserves_audio_state_without_scroll_jump(self) -> None:
        ...
        self.assertIn("let pendingFullscreenTransfer = false;", script)
        ...
        self.assertIn("pendingFullscreenTransfer = true;", script)
