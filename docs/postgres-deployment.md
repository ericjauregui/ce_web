...
The shareable site analytics dashboard is available publicly at `/admin/analytics` with no login or password. Anyone who has the link can view aggregate totals and grouped page, click, referrer, campaign, and Connect-page counts. The dashboard does not return session hashes or individual session journeys. It reads `site_analytics_events` and the existing aggregate `connect_event_counts` table through the web service's `DATABASE_URL`, uses `private, no-store` responses, and excludes dashboard visits from both site and Cloudflare analytics beacons.
...
Run the browser suite with `uv run python -m tests.run e2e` and verify its evidence with `uv run python -m tests.verify_e2e_artifacts --check-source`. Run `uv run python -m tests.run standard` for the retained isolation checks. PostgreSQL checks are opt-in via `TEST_DATABASE_URL` and require a dedicated, migrated test database; never point them at production. Use `DATABASE_URL` for the migration command in that test shell, then `TEST_DATABASE_URL` for the test runner. Integration tests use synthetic data and clean up their own records.
